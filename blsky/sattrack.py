...
class Track(ephem.BaseEphem):
    ...
    def __init__(self, fname, store_file=True):
        to_add = ['since', 'lon', 'lat', 'az', 'el']
        self.param += to_add  # add variable you want to include in the class - must be before super()  # noqa
        self.fname = fname
    ...
    def read_file(self):
        """
        Read in the .out file generated by satpos (or by rewrite_file).
        """
        ...
        with open(self.fname, 'r') as fp:
            for line in fp:
                if self.store_file:
                    self.archivelines.append(line.strip())
                if line[0] == '#':
                    if line.startswith('#rewrite:'):
                        extpar = ['lon', 'lat', 'az', 'el']
                    self.headerlines.append(line.strip()[1:])
                    continue
                data = line.strip().split()
                df = [float(data[i]) for i in range(len(satpar))]
                if len(extpar):
                    df += [float(data[i]) for i in range(18, 18+len(extpar))]
                for i, sp in enumerate(satpar + extpar):
    ...
    def rewrite_file(self, overwrite=False):
        """
        This rewrites the datafile, adding subsat point, az, el.

        Assumes that self.view has been executed.
        """
        if not self.store_file or not len(self.archivelines) or not self.viewed:
            print("Need to store_file and view first!")
            return
        if overwrite:
            newfile = self.fname
            ...
            newfile = f"{self.fname}.update"
        with open(newfile, 'w') as fp:
            i = 0
            print(f"#rewrite: {Time.now().isot}", file=fp)
            for arcline in self.archivelines:
                if arcline[0] == '#':
                    print(f"{arcline}", file=fp)
                else:
                    print(f"{arcline}\t"
                          f"{self.lon[i].value:11.6f}  {self.lat[i].value:11.6f}  "
                          f"{self.az[i].value:11.6f}  {self.el[i].value:11.6f}",
                          file=fp)
                    i += 1
